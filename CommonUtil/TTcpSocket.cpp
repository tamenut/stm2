...
//수신용 socket (server)
TTcpSocket::TTcpSocket(unsigned short bind_port)
:_user_data_queue(1024 * 1024 * 5)
{
...
//송신용 socket (client)
TTcpSocket::TTcpSocket(const char *dst_ip_str, unsigned short bind_port)
{
...
void TTcpSocket::sender_init(int dst_ip_addr, unsigned short bind_port)
{
...
void TTcpSocket::receiver_init(unsigned short bind_port)
{
	bool res_bind = true;
...
void TTcpSocket::set_pkt_len_offset(unsigned short pkt_size_start_offset, unsigned short pkt_size_length)
{
	_pkt_size_start_offset = pkt_size_start_offset;
	_pkt_size_length = pkt_size_length;
}

void TTcpSocket::set_pkt_len_offest_byte_ordering(bool network_byte_ordering_flag)
{
	_is_network_byte_ordering = network_byte_ordering_flag;
}

bool TTcpSocket::connection()
{
...
int TTcpSocket::read(char *payload, unsigned int payload_len)
{
	int len = -1;
...
int TTcpSocket::read(SerializedPayload &serialized_payload)
{
...
int TTcpSocket::read_and_push_user_data_queue(SOCKET client_sock)
{
	int pkt_len=0;
...
void TTcpSocket::push_user_data_queue(char *payload, int payload_len)
{
	if (_recv_blocking == false)
	{
		...
		if (_user_data_queue.push_string(payload, payload_len) == false)
		{
			...
			printf("Error : TTcpSocket Buffer is Full - Packet is droped\n");
		}
		_queue_cond.signal();
		_queue_lock.unlock();
	}
	else
	{
		...
		do
		{
			...
			if (res == false)
			{
				...
				if (cnt++ / 1000 == 0)
				{
					printf("Error : TTcpSocket Buffer is Full - Blocking(block count:%u)\n", cnt);
				}
}
...
int TTcpSocket::pop_user_data_queue(char *payload, unsigned int payload_len)
{
...
int TTcpSocket::post(SerializedPayload &serialized_payload)
{
	return post(serialized_payload.get_payload_ptr(), serialized_payload.get_payload_len());
}

//tcp 소켓으로 전송하는 함수
int TTcpSocket::post(char *payload, unsigned int payload_len)
{
	int sent_len = -1;
...
int TTcpSocket::_post(char *payload, int payload_len)
{
	int sent_len = 0;
	int sent_result;
...
void TTcpSocket::set_recv_blocking(bool recv_blocking)
{
	_recv_blocking = recv_blocking;
}

void TTcpSocket::set_snd_time_out(int milisec_time_out)
{
#ifdef WIN32
	if (_snd_sock != NULL)
	{
...
void TTcpSocket::set_rcv_time_out(int milisec_time_out)
{
	if (_rcv_sock)
	{
#ifdef WIN32
...
bool TTcpSocket::is_connection()
{
	return _connection_flag;
}

void TTcpSocket::set_linger()
{
	if (_rcv_sock != NULL)
	{
...
unsigned int TTcpSocket::get_current_rcv_buf_size()
{
	return _user_data_queue.get_string_size();
}

unsigned int TTcpSocket::get_current_rcv_buf_msg_cnt()
{
