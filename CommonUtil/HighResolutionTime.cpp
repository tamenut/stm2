...
bool HighResolutionTime::Start(void)
{
	...
	// 1frequency가 실행되는 시간(per second) : 너무 작은값이므로 보정값을 곱해준다.
	_resolution = ((double)HIGHRESOULTION_BASE/frequency.QuadPart);
...
double HighResolutionTime::get_elapsed_time(int unit_resolution)
{
	LARGE_INTEGER currentTime;
	...
	return (((double)(currentTime.QuadPart - start_time.QuadPart)*_resolution)/unit_resolution);
}
*/
