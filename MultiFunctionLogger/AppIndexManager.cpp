...
namespace MF_LOG
{
AppIndexManager* AppIndexManager::instance;
/*
싱글톤 처리를 위한 함수
*/
AppIndexManager* AppIndexManager::Instance()
...
*/
int AppIndexManager::getAppIndex(int domainId)
{
	int allocIndex = 0;
	...
	{	//새로운 도메인 정보를 생성
		...
		sprintf( shmName, "LOG_INDEX_SHM_%d", domainId );
		...
		printf("AppIndexManager::getAppIndex new DomainInfo\n");
	...
	{
		unlock();
		printf("AppIndexManager::getAppIndex allocIndexSize overflow : %d", MAX_ALLOCINDEX_SIZE);
	}
	
	unlock();
	//2011.04.25
	printf("AppIndexManager::getAppIndex allocIndex : %d\n", allocIndex);
	return allocIndex;
}

#if defined(LINUX)
int AppIndexManager::get_count_openfile(const char* name)
{
	int count = 0;
	DIR *dir;
